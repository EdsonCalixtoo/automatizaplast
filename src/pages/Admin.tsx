...
const Admin = () => {
  ...
  const handleAddProduct = () => {
    setSelectedProduct({
      ...
      old_price: 4500,
      tag: "LANÇAMENTO",
      badge: "QUALIDADE ABS INDUSTRIAL",
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, field: 'image' | 'gallery') => {
    ...
    if (field === 'image') {
      setSelectedProduct({ ...selectedProduct, image: uploadedUrls[0] });
    } else {
      const currentGallery = Array.isArray(selectedProduct.gallery) ? selectedProduct.gallery : [];
      setSelectedProduct({ ...selectedProduct, gallery: [...currentGallery, ...uploadedUrls] });
    }
    setLoading(false);
  };
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    // Sanitização de dados e cálculo automático de parcelamento
    const price = Number(selectedProduct.price) || 0;
    const old_price = selectedProduct.old_price ? Number(selectedProduct.old_price) : null;
    const installmentValue = (price / 5).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });

    const productToSave = { 
      ...selectedProduct,
      price: price,
      old_price: old_price,
      installment: `5x de ${installmentValue} sem juros`, // Gerado automaticamente
      gallery: Array.isArray(selectedProduct.gallery) ? selectedProduct.gallery : [],
      description: selectedProduct.description || selectedProduct.desc || ""
    };

    // Remove campos que não existem mais
    delete (productToSave as any).desc;
    ...
    if (!error) {
      ...
      if (index > -1) {
        ...
        newProducts[index] = productToSave;
        setCurrentProducts(newProducts);
      } else {
        setCurrentProducts([productToSave, ...currentProducts]);
      }
  ...
  const handlePrintNota = (order: any) => {
    ...
    const paymentInfo = order.payment_method || 'A DEFINIR';
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .header { display: flex; justify-content: space-between; align-items: center; background: #0a1e36; color: white; padding: 30px; border-radius: 20px; margin-bottom: 40px; }
            .logo { height: 50px; }
        ...
        <body>
          <div class="header">
            ...
            <div style="text-align: right">
              <h2 style="margin: 0; color: white;">PEDIDO DE COMPRA</h2>
  ...
  if (loading && !authorized) {
    return (
      <div className="min-h-screen bg-[#0a1e36] flex flex-col items-center justify-center text-white gap-4">
        <Loader2 className="w-10 h-10 animate-spin text-primary" />
        <p className="font-bold uppercase tracking-widest text-xs opacity-50">Verificando Credenciais...</p>
      </div>
    );
  }
      ...
      <aside className="w-72 bg-[#0a1e36] text-white flex flex-col fixed inset-y-0 hidden lg:flex">
        ...
        <div className="p-8 border-t border-white/5">
           <button 
             ...
             className="w-full h-16 flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-all text-left"
           >
          ...
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {activeTab === "dashboard" && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  { label: "Faturamento Real", value: `${(orders.reduce((sum, o) => sum + (Number(o.total_price) || 0), 0)).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`, trend: "Atualizado", icon: TrendingUp, color: "text-emerald-500" },
                  { label: "Total de Pedidos", value: `${orders.length}`, trend: "Real", icon: ShoppingBag, color: "text-primary" },
                  { label: "Ticket Médio", value: `${(orders.length > 0 ? orders.reduce((sum, o) => sum + (Number(o.total_price) || 0), 0) / orders.length : 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`, trend: "Calculado", icon: Users, color: "text-blue-500" },
                  { label: "Aguardando Pagto", value: `${orders.filter(o => o.status?.includes('Aguardando')).length}`, trend: "Atenção", icon: Clock, color: "text-orange-500" },
                ].map((stat) => (
                  <div key={stat.label} className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 hover:shadow-xl hover:translate-y-[-4px] transition-all duration-300">
                     <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-[10px] font-black bg-slate-50 px-3 py-1 rounded-full text-slate-400">{stat.trend}</span>
                     </div>
                     <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">{stat.label}</p>
                     <h3 className="text-2xl font-black text-[#0a1e36]" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>{stat.value}</h3>
                  </div>
                ))}
              </div>
            )}
            ...
            {activeTab === "dashboard" && (
              <div className="grid lg:grid-cols-3 gap-8">
                 <div className="lg:col-span-2 bg-[#0a1e36] text-white p-8 rounded-[40px] shadow-2xl relative overflow-hidden">
                    ...
                    <h3 className="text-2xl font-black uppercase mb-8 flex items-center gap-3" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>Vendas Recentes</h3>
                    <div className="h-64 w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={orders.slice(0, 7).map((o, i) => ({ name: `Ped ${i+1}`, vendas: o.total_price }))}>
                          <defs>
                 ...
                 <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
                    <h3 className="text-2xl font-black uppercase mb-6 text-[#0a1e36]" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>Distribuição</h3>
                    <div className="space-y-6">
                      {[
                        {status: "Pagos", value: orders.filter(o => o.status === 'Pago').length, col: "bg-emerald-500"}, 
                        {status: "Processando", value: orders.filter(o => o.status?.includes('Processando')).length, col: "bg-primary"}, 
                        {status: "Pendentes", value: orders.filter(o => o.status?.includes('Aguardando')).length, col: "bg-orange-500"}
                      ].map(item => (
                        <div key={item.status} className="space-y-2">
                           <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-slate-400">
                              <span>{item.status}</span>
                              <span className="text-[#0a1e36] font-bold">{item.value}</span>
                           </div>
                           <div className="w-full h-2 bg-slate-50 rounded-full overflow-hidden">
                              <div className={`h-full ${item.col}`} style={{width: `${(orders.length > 0 ? (item.value/orders.length)*100 : 0)}%`}}></div>
                           </div>
                        </div>
                      ))}
                    </div>
             ...
             <div className="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl relative overflow-hidden">
                ...
                <form onSubmit={handleSaveProduct} className="p-10 space-y-8 overflow-y-auto max-h-[70vh]">
                   <div className="grid grid-cols-2 gap-6">
                      ...
                      <div className="space-y-2">
                         <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Badge (Topo Detalhes)</label>
                         <input 
                           placeholder="Ex: QUALIDADE ABS INDUSTRIAL"
                           className="w-full bg-slate-50 border-none h-14 rounded-2xl px-6 focus:ring-2 focus:ring-primary/20 text-sm font-bold"
                   ...
                   <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2">
                         <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Preço Atual</label>
                         <div className="relative">
                            <input 
                              ...
                              placeholder="R$ 0,00"
                              className="w-full bg-slate-50 border-none h-14 rounded-2xl px-6 focus:ring-2 focus:ring-primary/20 text-sm font-bold"
                              value={new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(selectedProduct?.price || 0)}
                              onChange={(e) => {
                                const val = e.target.value.replace(/\D/g, "");
                                setSelectedProduct({...selectedProduct, price: Number(val) / 100});
                              }}
                            />
                         </div>
                      </div>
                      <div className="space-y-2">
                         <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Preço Antigo (R$)</label>
                         <input 
                           type="text"
                           placeholder="R$ 0,00"
                           className="w-full bg-slate-50 border-none h-14 rounded-2xl px-6 focus:ring-2 focus:ring-primary/20 text-sm font-bold"
                           value={new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(selectedProduct?.old_price || 0)}
                           onChange={(e) => {
                              const val = e.target.value.replace(/\D/g, "");
                              setSelectedProduct({...selectedProduct, old_price: Number(val) / 100});
                           }}
                         />
                      </div>
                   </div>
                   ...
                   <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2">
                         <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Destaque / Tag (Catálogo)</label>
                         <input 
                      ...
                      <div className="space-y-2">
                         <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Condição Comercial</label>
                         <div className="h-14 bg-slate-100/50 rounded-2xl flex items-center px-6">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                               Calculado: 5x sem juros + 12x com juros
                            </span>
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Imagem Principal (Capa)</label>
                        <div className="flex flex-col gap-2">
                          ...
                          />
                          {selectedProduct?.image && <p className="text-[9px] text-emerald-500 font-bold truncate">✓ Arquivo Selecionado</p>}
                        </div>
                      </div>
                      <div className="space-y-2">
                         <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Adicionar na Galeria (Fotos/Vídeos)</label>
                         <div className="flex flex-col gap-2">
                           <input 
                             type="file" 
                             multiple 
                             ...
                             className="text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20"
                           />
                           <p className="text-[9px] text-slate-400">Total: {selectedProduct?.gallery?.length || 0} arquivos na galeria</p>
                         </div>
                      </div>
                   </div>

                   <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Descrição Completa</label>
                      <textarea 
                        required
                        rows={4}
                        className="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 focus:ring-2 focus:ring-primary/20 text-sm font-medium"
                        value={selectedProduct?.description || selectedProduct?.desc || ""}
